...
SET od=W:\Lab4\data\aster_elev\hydro

:: the following creates the output directory if it doesn't exist already
if not exist %od% mkdir %od%

saga_cmd ta_lighting 0 -ELEVATION=W:\Lab4\data\aster_elev\Lab4_ASTERElevmosaicUTM.sgrd -SHADE=%od%\AnalyticalHillshade.sgrd -METHOD=0 -POSITION=0 -AZIMUTH=315 -DECLINATION=45 -EXAGGERATION=1 -UNIT=0

saga_cmd ta_preprocessor 1 -ELEVATION=W:\Lab4\data\aster_elev\Lab4_ASTERElevmosaicUTM.sgrd -SINKROUTE=%od%\SinkRoute.sgrd -THRESHOLD=0 -THRSHEIGHT=100

saga_cmd ta_preprocessor 2 -DEM=W:\Lab4\data\aster_elev\Lab4_ASTERElevmosaicUTM.sgrd -SINKROUTE=%od%\SinkRoute.sgrd -DEM_PREPROC=%od%\RemovedSinks.sgrd -METHOD=1 -THRESHOLD=0 -THRSHEIGHT=100
